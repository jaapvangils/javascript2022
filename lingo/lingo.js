//const quicklist = require('./woorden.js');
...
var gameloop = function () {
  // pick a random word
  var rand =
    quicklist[Math.floor(Math.random() * quicklist.length)].toUpperCase();
  ...
  input.onkeypress = function (event) {
    if (event.key == "Enter" || event.keyCode == 13) {
      document.getElementById("smallMsg").innerHTML =
        "Green = correct letter, Yellow = wrong place"; // reset message
      ...
      for (var i = 0; i < childDivs.length; i++) {
        ...
        if (c === 5) {
          // if they have all the correct letters
      ...
      for (var i = 0; i < childDivs.length; i++) {
        if (rand.indexOf(guess[i]) != -1) {
          if (
            hasDuplicates === false &&
            childDivs[rand.indexOf(guess[i])].className != "square correct"
          ) {
          ...
          // if there are duplicate letters
            ...
            if (ind.length > 1) {
              for (var j = 0; j < ind.length; j++) {
                if (
                  childDivs[ind[j]].className != "square correct" &&
                  childDivs[i].className != "square wrongplace"
                ) {
                  changeClass(childDivs[i], "default", "wrongplace");
                } //if
              } //for
...
var playagain = function () {
  ...
  document.getElementById("smallMsg").innerHTML =
    "Green = correct letter, Yellow = wrong place"; // small message
  ...
  for (var i = 1; i < 6; i++) {
    ...
    for (var j = 0; j < 5; j++) {
      resets[j].innerHTML = "";
      if (
        resets[j].className == "square correct" ||
        resets[j].className == "square wrongplace"
      ) {
